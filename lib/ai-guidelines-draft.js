...
export function resolvePlatformOptions(doc, { generationOnly = false } = {}) {
  ...
  if (generationOnly) {
    ...
    options = PLATFORMS.filter((id) => allowed.has(id) && byId.has(id)).map((id) => ({
      id,
      label: byId.get(id)?.label || PLATFORM_LABELS[id] || id,
    }))
    if (!options.length) {
...
export function resolveContentTypeOptions(doc) {
  ...
  const knownIds = new Set(CONTENT_TYPES)
  const orderedIds = [
    ...CONTENT_TYPES,
    ...fromDoc.map(({ id }) => id).filter((id) => !knownIds.has(id)),
  ]
