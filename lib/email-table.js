...
/**
 * Gmail search URL for one message. Every group member holds a copy, so the
 * viewer's own Gmail finds it by RFC 822 Message-ID. `authuser` selects the
 * viewer's account when several are signed in.
 * @param {string|null} messageId - with or without angle brackets
 * @param {string} [viewerEmail]
 * @returns {string|null}
 */
export function gmailMessageUrl(messageId, viewerEmail) {
  if (!messageId) return null
  const id = String(messageId).replace(/^<|>$/g, '')
  const account = viewerEmail ? `?authuser=${encodeURIComponent(viewerEmail)}` : ''
  // in:anywhere also searches Spam and Trash, where blasts land for 30 days
  return `https://mail.google.com/mail/${account}#search/in%3Aanywhere+rfc822msgid:${encodeURIComponent(id)}`
}
