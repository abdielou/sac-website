...
import { OBSERVATION_NIGHT_CONTENT_TYPE, OBSERVATION_NIGHT_LABEL } from '../ai-constants'
...
export function buildTemplateTextFields({ input, contentType, posterText } = {}) {
  ...
  const eventName =
    type === OBSERVATION_NIGHT_CONTENT_TYPE
      ? OBSERVATION_NIGHT_LABEL
      : trimOrUndefined(input?.eventDetails?.name)
