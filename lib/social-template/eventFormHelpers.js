...
import { getCanonicalEventName, isEventContentType } from '../ai-constants'

export const EVENT_WEATHER_DISCLAIMER = '*Actividad sujeta a condiciones del tiempo'
 ...
 * @returns {string}
 */
export function resolveEventName(formState, definition) {
  return (
    getCanonicalEventName(formState.contentType, definition) || (formState.eventName || '').trim()
  )
}
 ...
 */
export function buildEventDetails(formState, definition) {
  if (!isEventContentType(formState.contentType, definition)) return undefined

  const details = { name: resolveEventName(formState, definition) }
 ...
 */
export function deriveEventTopicAndIntent(formState, definition) {
  const name = resolveEventName(formState, definition)
 ...
 * @param {string} cta
 * @param {{ requireCta?: boolean }} [options]
 * @returns {string[]} missing field labels
 */
export function missingEventLogistics(eventDetails, cta, { requireCta = true } = {}) {
  const missing = []
  ...
  if (requireCta && !String(cta || '').trim()) missing.push('CTA')
  return missing
}
