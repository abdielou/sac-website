...
export async function renderSocialTemplateImage({ templateRequest, platform: _platform }) {
  ...
  const layout = getTemplateLayout(
    templateRequest.layout,
    canvas,
    templateRequest.templatePresentation
  )
  if (!layout) {
  ...
  const preservesGeneratedBackground = templateRequest.backgroundSource?.mode === 'ai_generated'
  ...
  const [resizedBackground, logoComposite, sponsorComposite, fontFaceCss] = await Promise.all([
    sharp(backgroundBuffer, SAFE_SHARP_INPUT_OPTIONS)
      ...
      .resize({
        ...
        // Stock art is curated for the template crop. Provider output is not:
        // preserve the whole generated scene when it ignores the requested 3:4 ratio.
        fit: preservesGeneratedBackground ? 'contain' : 'cover',
        position: 'centre',
        ...(preservesGeneratedBackground ? { background: '#0B081C' } : null),
      })
      .png()
      .toBuffer(),
