...
export const REFRESH_SCOPES = {
  ...
  contacts: { cacheKeys: [CACHE_KEYS.CONTACTS], queryKeys: [['contacts']] },
  all: {
    cacheKeys: null,
    queryKeys: [['members'], ['payments'], ['contacts']],
  },
}

/**
 * Scope for an admin pathname. `/admin/contacts` gives `contacts`, the dashboard
 * and any section without its own cache give `all`.
 * @param {string|null|undefined} pathname
 * @returns {'members'|'payments'|'contacts'|'all'}
 */
