import { buildAiRunFailure } from './ai-run-failure'
import { buildUserKey } from './run-history-store'

const HISTORY_SCHEMA_VERSION = 2
const LEGACY_UNVERSIONED = 'legacy-unversioned'

function normalizeContentTypeIdentity(contentTypeIdentity) {
  if (
    !contentTypeIdentity ||
    typeof contentTypeIdentity !== 'object' ||
    Array.isArray(contentTypeIdentity)
  ) {
    return undefined
  }

  const identity = {}
  for (const key of ['id', 'label', 'guidelineVersion']) {
    if (typeof contentTypeIdentity[key] === 'string' && contentTypeIdentity[key]) {
      identity[key] = contentTypeIdentity[key]
    }
  }

  return Object.keys(identity).length > 0 ? identity : undefined
}

function applyVersionIdentityFields(record, { policyVersion, contentTypeIdentity }) {
  if (typeof policyVersion === 'string' && policyVersion) {
    record.policyVersion = policyVersion
  } else {
    record.policyVersion = LEGACY_UNVERSIONED
  }

  const normalizedIdentity = normalizeContentTypeIdentity(contentTypeIdentity)
  if (normalizedIdentity) {
    record.contentTypeIdentity = normalizedIdentity
  } else {
    record.contentTypeIdentity = LEGACY_UNVERSIONED
  }
}

/**
 * Read-time projection for immutable v1 history objects. It labels missing
 * version identity explicitly without pretending that a current version applied.
 */
export function normalizeRunHistoryRecord(record) {
  if (!record || typeof record !== 'object' || Array.isArray(record)) return record
  const normalized = { ...record }
  applyVersionIdentityFields(normalized, {
    policyVersion: record.policyVersion,
    contentTypeIdentity: record.contentTypeIdentity,
  })
  return normalized
}

function buildInputSummary(input) {
  if (!input || typeof input !== 'object') return {}

  return {
    platform: input.platform,
    platforms: Array.isArray(input.platforms) ? input.platforms : undefined,
    contentType: input.contentType,
    draftTextLength: typeof input.draftText === 'string' ? input.draftText.length : 0,
    imageCount: Array.isArray(input.images) ? input.images.length : 0,
    hasGoal: Boolean(input.goal),
    hasAudience: Boolean(input.audience),
    hasCta: Boolean(input.cta),
    hasEventDetails: Boolean(input.eventDetails),
    hasAltText: Boolean(input.altText),
  }
}

function buildGenerationInputSummary(input) {
  if (!input || typeof input !== 'object') return {}

  const platforms = Array.isArray(input.platforms) ? input.platforms : []

  return {
    platforms,
    contentType: input.contentType,
    intentLength: typeof input.intent === 'string' ? input.intent.length : 0,
    topicLength: typeof input.topic === 'string' ? input.topic.length : 0,
    hasTone: Boolean(input.tone),
    hasAudience: Boolean(input.audience),
    hasCta: Boolean(input.cta),
    hasKnownFacts: Array.isArray(input.knownFacts) && input.knownFacts.length > 0,
    hasEventDetails: Boolean(input.eventDetails),
    hasHashtags: Array.isArray(input.hashtags) && input.hashtags.length > 0,
    hasLinks: Array.isArray(input.links) && input.links.length > 0,
    hasImageStyle: Boolean(input.imageStyle),
    hasImageConstraints: Boolean(input.imageConstraints),
  }
}

function buildOutcomeSummary(result) {
  if (!result || typeof result !== 'object') return undefined

  const issues = Array.isArray(result.issues) ? result.issues : []
  const issueCounts = issues.reduce(
    (acc, issue) => {
      const sev = issue?.severity
      if (sev === 'critical' || sev === 'major' || sev === 'minor') {
        acc[sev] += 1
      }
      return acc
    },
    { critical: 0, major: 0, minor: 0 }
  )

  const summary = {
    overallOutcome: result.overallOutcome,
    approvalRecommendation: result.approvalRecommendation,
    issueCount: issues.length,
    issueCounts,
  }
  if (typeof result.resultSource === 'string') summary.resultSource = result.resultSource
  return summary
}

function buildGenerationOutcomeSummary(result) {
  if (!result || typeof result !== 'object') return undefined

  const drafts = Array.isArray(result.drafts) ? result.drafts : []
  const platforms = drafts.map((d) => d?.platform).filter(Boolean)
  const distinctCaptions = new Set(
    drafts.map((draft) => draft?.draftText).filter((draftText) => typeof draftText === 'string')
  )
  const hasMissingInformation = drafts.some(
    (d) => Array.isArray(d?.missingInformation) && d.missingInformation.length > 0
  )

  const summary = {
    draftCount: distinctCaptions.size,
    platforms,
    hasMissingInformation,
  }

  if (result.policyReview && typeof result.policyReview === 'object') {
    if (typeof result.policyReview.stage === 'string') {
      summary.policyStage = result.policyReview.stage
    }
    if (typeof result.policyReview.disposition === 'string') {
      summary.policyDisposition = result.policyReview.disposition
    }
    if (typeof result.policyReview.failClosed === 'boolean') {
      summary.policyFailClosed = result.policyReview.failClosed
    }
  }

  return summary
}

function applyUsageFields(record, usage, model) {
  if (usage?.openRouterGenerationId) {
    record.openRouterGenerationId = usage.openRouterGenerationId
  }
  if (typeof usage?.promptTokens === 'number') {
    record.promptTokens = usage.promptTokens
  }
  if (typeof usage?.completionTokens === 'number') {
    record.completionTokens = usage.completionTokens
  }
  if (typeof usage?.totalTokens === 'number') {
    record.totalTokens = usage.totalTokens
  }
  if (usage?.cost && typeof usage.cost.amount === 'number') {
    record.cost = {
      amount: usage.cost.amount,
      currency: usage.cost.currency || 'USD',
    }
  }
  if (usage?.model || model) {
    record.model = usage?.model || model
  }
}

function buildHistoryFailure(error) {
  const message =
    typeof error === 'string'
      ? error.slice(0, 200)
      : String(error?.message || 'failed').slice(0, 200)
  const isStructured =
    error &&
    typeof error === 'object' &&
    (error.schemaVersion === 1 || typeof error.code === 'string' || typeof error.stage === 'string')

  if (!isStructured) {
    return { message, retryable: Boolean(error?.retryable) }
  }

  return buildAiRunFailure({
    code: error.code,
    stage: error.stage,
    retryable: error.retryable,
    message,
  })
}

/**
 * Build sparse AiRunHistoryRecord for validation runs (no prompts, uploads, or secrets).
 */
export function buildValidationHistoryRecord({
  input,
  runId,
  status,
  result,
  error,
  failure,
  startedAt,
  completedAt,
  guidelineVersion,
  policyVersion,
  contentTypeIdentity,
  legacy = false,
  model,
  provider = 'openrouter',
  usage,
}) {
  const userId = input?.userId
  const userKey = userId ? buildUserKey(userId) : undefined
  const createdAt = startedAt || new Date().toISOString()
  const finishedAt = completedAt || new Date().toISOString()
  const durationMs =
    startedAt && completedAt
      ? Math.max(0, new Date(completedAt).getTime() - new Date(startedAt).getTime())
      : undefined

  const record = {
    schemaVersion: HISTORY_SCHEMA_VERSION,
    runId,
    mode: 'validate',
    status,
    userKey,
    createdAt,
    completedAt: finishedAt,
    durationMs,
    platform: input?.platform,
    contentType: input?.contentType,
    guidelineVersion,
    provider,
    inputSummary: buildInputSummary(input),
  }

  applyVersionIdentityFields(record, { policyVersion, contentTypeIdentity, legacy })
  applyUsageFields(record, usage, model)

  if (status === 'completed' && result) {
    record.validationOutcome = result.overallOutcome
    if (typeof result.resultSource === 'string') {
      record.validationResultSource = result.resultSource
    }
    record.outcomeSummary = buildOutcomeSummary(result)
  }

  const terminalFailure = failure || error
  if (status === 'failed' && terminalFailure) {
    record.error = buildHistoryFailure(terminalFailure)
  }

  return record
}

/**
 * Build sparse AiRunHistoryRecord for generation runs (no full prompts or draft text).
 */
export function buildGenerationHistoryRecord({
  input,
  runId,
  status,
  result,
  error,
  failure,
  startedAt,
  completedAt,
  guidelineVersion,
  policyVersion,
  contentTypeIdentity,
  legacy = false,
  model,
  provider = 'openrouter',
  usage,
}) {
  const userId = input?.userId
  const userKey = userId ? buildUserKey(userId) : undefined
  const createdAt = startedAt || new Date().toISOString()
  const finishedAt = completedAt || new Date().toISOString()
  const durationMs =
    startedAt && completedAt
      ? Math.max(0, new Date(completedAt).getTime() - new Date(startedAt).getTime())
      : undefined

  const platforms = Array.isArray(input?.platforms) ? input.platforms : undefined

  const record = {
    schemaVersion: HISTORY_SCHEMA_VERSION,
    runId,
    mode: 'generate',
    status,
    userKey,
    createdAt,
    completedAt: finishedAt,
    durationMs,
    platform: platforms?.length === 1 ? platforms[0] : undefined,
    contentType: input?.contentType,
    guidelineVersion,
    provider,
    inputSummary: buildGenerationInputSummary(input),
  }

  applyVersionIdentityFields(record, { policyVersion, contentTypeIdentity, legacy })
  applyUsageFields(record, usage, model)

  if (status === 'completed' && result) {
    record.outcomeSummary = buildGenerationOutcomeSummary(result)
  }

  const terminalFailure = failure || error
  if (status === 'failed' && terminalFailure) {
    record.error = buildHistoryFailure(terminalFailure)
  }

  return record
}
