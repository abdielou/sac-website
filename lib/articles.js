...
/**
 * Normalise a tag list so one concept maps to exactly one tag hub URL.
 *
 * The taxonomy fragmented because tags were stored exactly as typed. `eclipse`
 * and `Eclipse` became two hubs, and an accented tag such as `astronomía` became
 * the percent-encoded URL `/tags/astronom%C3%ADa`. Every write goes through this
 * helper, so the slug space stays consistent from now on.
 *
 * Singular and plural forms are deliberately NOT merged. `meteoro` and
 * `meteoros` are a content decision for the editor, not something code guesses.
 *
 * @param {Array<string>|string|undefined} tags - Tags as supplied by the caller
 * @returns {Array<string>} Lowercase, unaccented, trimmed, de-duplicated tags
 */
export function normaliseTags(tags) {
  ...
  for (const raw of list) {
    ...
    // readers by components/Tag.js, and 'bolido' for 'bólido' is a misspelling.
    // De-accenting belongs in the URL slug layer, not in the stored value.
    const tag = String(raw).trim().replace(/\s+/g, ' ').toLowerCase()
    if (tag && !normalised.includes(tag)) normalised.push(tag)
  }
...
export async function createArticle(data) {
  ...
  // Always a full ISO 8601 timestamp, never a bare YYYY-MM-DD.
...
/**
 * A published article must carry at least one tag.
 *
 * Tags are the only working internal hubs on this site, so an untagged published
 * article is orphaned: nothing links to it but chronological prev/next. 13 were
 * published that way before this guard existed. Drafts may stay untagged.
 *
 * @throws {Error} with a Spanish message suitable for returning to the editor.
 */
export function assertPublishable(article) {
  const isPublished = article.draft === false
  const hasTags = Array.isArray(article.tags) && article.tags.some((t) => String(t).trim())
  if (isPublished && !hasTags) {
    throw new Error('Un articulo publicado requiere al menos una etiqueta')
  }
}
...
export async function updateArticle(slug, updates) {
  ...
  // Same guarantee as createArticle: a full ISO 8601 timestamp on every write.
  updatedArticle.lastmod = toIso(Date.now())
  // Re-normalise on every edit, so an article saved before this rule is healed.
  updatedArticle.tags = normaliseTags(updatedArticle.tags)

  // Checked on the merged result, not on `updates`: a partial edit that only
  // flips draft to false must still be held to the rule.
  assertPublishable(updatedArticle)
