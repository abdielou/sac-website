...
export function isStaleGenerationRun(payload, now = Date.now()) {
  if (!payload || (payload.status !== 'pending' && payload.status !== 'running')) {
    return false
  }

  const activityTimes = [payload.updatedAt, payload.startedAt, payload.createdAt]
    .map((value) => (value ? new Date(value).getTime() : Number.NaN))
    .filter(Number.isFinite)

  return activityTimes.length > 0 && now - Math.max(...activityTimes) > STALE_RUN_MS
}
  ...
  const pollRun = useCallback(
    async (id) => {
      ...
      try {
        ...
        if (isStaleGenerationRun(data)) {
          abortRef.current = true
          setRunId(null)
          setPhase('failed')
          setError(
            'La ejecución anterior se interrumpió y ya no está activa. Puedes volver a generar.'
          )
          setResult(null)
          setUsage(null)
          setGuidelineVersion(null)
    ...
    [applyCompletedPayload, clearPolling, setRunIdInUrl]
  )
