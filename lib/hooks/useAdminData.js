...
import { REFRESH_SCOPES, refreshScopeFor } from '../admin/refresh-scope'
...
export function useRefreshData() {
  ...
  // The header button refreshes only the section on screen: emails on
  // /admin/emails, payments on /admin/payments, and so on. Sections without
  // a server cache fall back to a full refresh.
  const scope = refreshScopeFor(pathname)
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      // Step 1: Clear the server cache for this section
      ...
      // Step 2: Invalidate the matching queries (triggers refetch)
      for (const queryKey of REFRESH_SCOPES[scope].queryKeys) {
  ...
  return { refresh, isRefreshing, error, scope }
}
...
export async function verifyMemberSacEmail(email, sacEmail) {
  ...
  const member = membersData.data?.find((m) => m.email?.toLowerCase() === email.toLowerCase())
...
async function fetchEmailAccountability(forceRefresh) {
  const params = new URLSearchParams()
  ...
  if (!res.ok) {
    const error = await res.json().catch(() => ({ error: 'Unknown error' }))
    throw new Error(error.details || error.error || 'Failed to fetch emails')
  }
...
/**
 * Fetch the email accountability threads for the group address.
 * `refresh()` refetches with the server cache bypassed and writes the result
 * into the single shared `['emails']` entry, so every instance of the hook
 * (dashboard card and emails page) shows the same data.
 */
export function useEmailAccountability({ enabled = true } = {}) {
  const queryClient = useQueryClient()

  const query = useQuery({
    queryKey: EMAILS_QUERY_KEY,
    enabled,
    queryFn: () => fetchEmailAccountability(false),
  })
  ...
  const refresh = useCallback(
    () =>
      queryClient
        .fetchQuery({
          queryKey: EMAILS_QUERY_KEY,
          queryFn: () => fetchEmailAccountability(true),
          staleTime: 0,
        })
        // The error is already stored on the query; observers read it there.
        .catch(() => {}),
    [queryClient]
  )
