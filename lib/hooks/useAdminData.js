...
export function useRefreshData() {
  ...
  // /admin/contacts, payments on /admin/payments, and so on. Sections without
...
async function fetchContacts(forceRefresh) {
  const params = new URLSearchParams()
  ...
  if (!res.ok) {
    const error = await res.json().catch(() => ({ error: 'Unknown error' }))
    throw new Error(error.details || error.error || 'Failed to fetch contacts')
  }
 ...
 * into the single shared `['contacts']` entry, so every instance of the hook
