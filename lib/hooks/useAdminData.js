...
export function useRefreshData() {
  ...
  // /admin/inquiries, payments on /admin/payments, and so on. Sections without
...
async function fetchInquiries(forceRefresh) {
  const params = new URLSearchParams()
  ...
  if (!res.ok) {
    const error = await res.json().catch(() => ({ error: 'Unknown error' }))
    throw new Error(error.details || error.error || 'Failed to fetch inquiries')
  }
 ...
 * into the single shared `['inquiries']` entry, so every instance of the hook
