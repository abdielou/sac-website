...
function buildFormData(formState, images, contentTypeDefinition, platforms) {
  ...
  fd.set('platforms', JSON.stringify(resolvedPlatforms))
...
export function useAiValidationRun({ canValidate }) {
  ...
  const submitValidation = useCallback(
    async (formState, images, contentTypeDefinition, platforms) => {
      ...
      try {
        const fd = buildFormData(formState, images, contentTypeDefinition, platforms)
