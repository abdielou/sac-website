...
function buildFormData(formState, images, contentTypeDefinition) {
  ...
  if (contentTypeDefinition) {
    ...
    if (
      !contentTypeAcceptsImages(formState.platform, formState.contentType, contentTypeDefinition)
    ) {
      delete contentData.sponsor
    }
    fd.set('contentData', JSON.stringify(contentData))
  }

  if (contentTypeAcceptsImages(formState.platform, formState.contentType, contentTypeDefinition)) {
...
export function useAiValidationRun({ canValidate }) {
  ...
  const [contentTypeIdentity, setContentTypeIdentity] = useState(null)
  const [error, setError] = useState(null)
  const [copyFeedback, setCopyFeedback] = useState(null)

  const applyCompletedPayload = useCallback((payload) => {
    // New shape: { result, usage }. Legacy runs may return AiValidationResult directly.
    const isWrapped =
      payload &&
    ...
    if (isWrapped) {
      ...
      setPolicyVersion(payload.policyVersion ?? LEGACY_POLICY_VERSION)
      setContentTypeIdentity(payload.contentTypeIdentity ?? null)
    } else {
      setResult(payload)
      setUsage(null)
      setGuidelineVersion(null)
      setPolicyVersion(LEGACY_POLICY_VERSION)
      setContentTypeIdentity(null)
    }
  }, [])
  ...
  const submitValidation = useCallback(
    async (formState, images, contentTypeDefinition) => {
      ...
      try {
        const fd = buildFormData(formState, images, contentTypeDefinition)
