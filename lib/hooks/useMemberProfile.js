...
/**
 * Mutation hook for uploading a family member photo from the member profile.
 * Calls POST /api/member/family/[familyName]/photo
 *
 * Usage:
 *   const { mutate, isPending } = useUploadMemberFamilyPhoto()
 *   mutate({ familyDisplayName: 'María López', photo: blob })
 *
 * @returns {import('@tanstack/react-query').UseMutationResult}
 */
export function useUploadMemberFamilyPhoto() {
  const queryClient = useQueryClient()

  return useMutation({
    mutationFn: async ({ familyDisplayName, photo }) => {
      const formData = new FormData()
      formData.append('photo', photo, 'photo.jpg')

      const res = await fetch(`/api/member/family/${encodeURIComponent(familyDisplayName)}/photo`, {
        method: 'POST',
        body: formData,
      })
