...
function genericContentTypeShape(platformIds = DEFAULT_SEED_PLATFORMS) {
  const resolvedPlatformIds =
    Array.isArray(platformIds) && platformIds.length ? platformIds : DEFAULT_SEED_PLATFORMS

  return {
    fields: GENERIC_FIELDS.map(([key, required]) => makeField(key, required)),
    titleSource: 'topic',
    visual: {
      mode: 'template',
      template: 'simple',
...
export function normalizeGuidelineDocumentV3(doc) {
  ...
  const contentTypeCatalog = doc.contentTypeCatalog.map((entry) => {
    ...
    const defaultPlatforms = configuredPlatformIds
...
export function createContentType(doc, { id, label = 'Nuevo tipo' } = {}) {
  ...
  const shape = genericContentTypeShape(platformIds)
  return {
    ...normalized,
    contentTypeCatalog: [
