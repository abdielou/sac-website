...
function seedShape(id, platformIds = DEFAULT_SEED_PLATFORMS) {
  ...
  const regularPostPolicies = Object.fromEntries(
    resolvedPlatformIds.map((platform) => {
      if (id !== 'regular_post') return [platform, 'optional']
      if (platform === 'x') return [platform, 'prohibited']
      if (platform === 'instagram') return [platform, 'required']
...
function mergeLegacyPlatformExpectations(validationRules, generationRules) {
  const validation = typeof validationRules === 'string' ? validationRules.trim() : ''
  const generation = typeof generationRules === 'string' ? generationRules.trim() : ''

  if (!validation) return generation
  if (!generation) return validation
  if (validation === generation) return validation
  if (generation.includes(validation)) return generation
  if (validation.includes(generation)) return validation

  // Older schemas asked editors to describe the same desired result twice. Preserve both texts
  // during migration, but present and store them as one neutral expectation from this point on.
  return `${generation}\n\n${validation}`
}
...
function removeLegacyCaptionLimit(rule) {
  if (typeof rule !== 'string') return rule
  return rule
    .replace(
      /\s*[,;:]?\s*(?:máximo(?:\s+de)?|límite(?:\s+de)?)\s+\d{1,5}\s+caracteres?\s*[,;.]?/gi,
      ' '
    )
    .replace(/\s+([,.;:])/g, '$1')
    .replace(/[ \t]{2,}/g, ' ')
...
function migratePlatformExpectations(doc, platformIds) {
  return Object.fromEntries(
    platformIds.map((id) => [
      id,
      mergeLegacyPlatformExpectations(
        removeLegacyCaptionLimit(doc?.platforms?.[id]),
        removeLegacyCaptionLimit(doc?.generation?.platforms?.[id])
      ),
    ])
  )
}
...
function normalizePlatformConstraints(doc, platformIds) {
  ...
  return Object.fromEntries(
    platformIds.map((id) => {
      ...
      // Older documents expressed this in prose. Migrate an explicit legacy limit, but never
      // infer one from the platform name alone. Once this field exists, null means no limit.
      return [
        id,
        {
          captionMaxCharacters: !hasExplicitConstraints
            ? extractCaptionCharacterLimit(doc?.platforms?.[id], doc?.generation?.platforms?.[id])
            : null,
        },
      ]
    })
...
function customLegacyIds(doc) {
  const known = new Set(CONTENT_TYPE_DEFINITIONS.map(({ id }) => id))
  const ids = new Set([
    ...Object.keys(doc?.contentTypes || {}),
    ...Object.keys(doc?.generation?.contentTypes || {}),
  ])
  return [...ids].filter((id) => !known.has(id))
}

export function migrateGuidelineDocumentToV3(doc) {
  ...
  const { platforms: legacyPlatforms, platformLabels } = normalizePlatforms(doc)
  const platformIds = Object.keys(legacyPlatforms)
  const platforms = migratePlatformExpectations(doc, platformIds)
  const platformConstraints = normalizePlatformConstraints(doc, platformIds)
  const hasCatalog = Array.isArray(doc.contentTypeCatalog)
  const catalog = hasCatalog
    ? clone(doc.contentTypeCatalog)
    : createDefaultContentTypeCatalog({
        validationRules: doc.contentTypes || {},
        generationRules: doc.generation?.contentTypes || {},
        platformIds,
      })

  for (const id of hasCatalog ? [] : customLegacyIds(doc)) {
    const label = id
      .split(/[_-]+/)
      .filter(Boolean)
      .map((part) => part[0]?.toUpperCase() + part.slice(1))
      .join(' ')
    const shape = seedShape(id, platformIds)
    catalog.push({
      id,
      label: label || id,
      status: 'active',
      description: `Contenido social de SAC: ${label || id}.`,
      fields: shape.fields,
      titleSource: shape.titleSource,
      validation: { rules: doc.contentTypes?.[id] || '' },
      generation: { rules: doc.generation?.contentTypes?.[id] || '' },
      visual: shape.visual,
    })
  }
...
export function normalizeGuidelineDocumentV3(doc) {
  ...
  const platformConstraints = normalizePlatformConstraints(doc, Object.keys(platforms))
    ...
    visual: entry?.visual
      ? {
          ...
          imagePolicyByPlatform:
            ...
            typeof entry.visual.imagePolicyByPlatform === 'object'
              ? { ...entry.visual.imagePolicyByPlatform }
              : entry.visual.imagePolicyByPlatform,
        }
      : entry?.visual,
...
// Transitional aliases for callers outside this module. New code should use the schema-neutral
// names once the broader Guidelines refactor lands.
export const migrateGuidelineDocumentToV2 = migrateGuidelineDocumentToV3
export const normalizeGuidelineDocumentV2 = normalizeGuidelineDocumentV3
...
export function validateGuidelineForActivation(doc, { baseDocument } = {}) {
  ...
  if (!nonEmptyText(normalized.generation?.global)) {
    report('required', 'generation.global', 'La voz global de generación es obligatoria.')
  } else if (exceedsTextLimit(normalized.generation.global, 20_000)) {
    report(
      'max_length',
      'generation.global',
      'La voz global de generación admite hasta 20000 caracteres.'
    )
  }
  ...
  for (const [entryIndex, entry] of catalog.entries()) {
    ...
    if (entry.id === OBSERVATION_NIGHT_CONTENT_TYPE) {
      if (entry.status !== 'active') {
        report(
          'protected_value',
          `${entryPath}.status`,
          'observation_night debe permanecer activo hasta una decisión autorizada.'
        )
      }
      if (entry.label !== OBSERVATION_NIGHT_LABEL) {
        report(
          'protected_value',
          `${entryPath}.label`,
          `observation_night debe conservar la etiqueta "${OBSERVATION_NIGHT_LABEL}".`
        )
      }
      if (entry.titleSource !== 'type_label') {
        report(
          'protected_value',
          `${entryPath}.titleSource`,
          'observation_night debe usar su etiqueta canónica como título.'
        )
      }
      const observationFields = new Map(
        (Array.isArray(entry.fields) ? entry.fields : []).map((field, index) => [
          field.key,
          { field, index },
        ])
      )
      for (const key of ['date', 'time', 'location']) {
        const observationField = observationFields.get(key)
        if (observationField?.field?.required !== true) {
          report(
            'protected_value',
            observationField
              ? `${entryPath}.fields.${observationField.index}.required`
              : `${entryPath}.fields`,
            `observation_night debe conservar ${key} como campo requerido.`
          )
        }
      }
      for (const key of ['cta', 'sponsor']) {
        const observationField = observationFields.get(key)
        if (!observationField || observationField.field?.required !== false) {
          report(
            'protected_value',
            observationField
              ? `${entryPath}.fields.${observationField.index}.required`
              : `${entryPath}.fields`,
            `observation_night debe conservar ${key} como campo opcional.`
          )
        }
      }
      if (observationFields.has('event_name')) {
        report(
          'protected_value',
          `${entryPath}.fields.${observationFields.get('event_name').index}`,
          'observation_night no puede sustituir su identidad con event_name.'
        )
      }
      if (entry.visual?.mode !== 'template' || entry.visual?.template !== 'event') {
        report(
          'protected_value',
          `${entryPath}.visual.template`,
          'observation_night debe conservar la plantilla event.'
        )
      }
      if (entry.visual?.sponsorAllowed !== true) {
        report(
          'protected_value',
          `${entryPath}.visual.sponsorAllowed`,
          'observation_night debe conservar el auspiciador opcional.'
        )
      }
    }
  }

  if (!ids.has(OBSERVATION_NIGHT_CONTENT_TYPE)) {
    report(
      'protected_value',
      'contentTypeCatalog',
      'observation_night debe conservarse como un tipo propio.'
    )
  }
  ...
  if (baseDocument) {
    const base = normalizeGuidelineDocumentV3(baseDocument)
    for (const entry of base?.contentTypeCatalog || []) {
      if (!ids.has(entry.id)) {
        report(
          'published_type_removed',
          'contentTypeCatalog',
          `El tipo publicado "${entry.id}" no puede eliminarse; debe archivarse.`
        )
      }
    }
  }
...
function uniqueId(base, ids) {
  ...
  let candidate = /^[a-z]/.test(normalized) ? normalized : `type_${normalized}`
  let suffix = 2
...
export function diffGuidelineDocuments(active, draft) {
  ...
  return { created, archived, restored, changed, reordered }
}
...
export function summarizeGuidelineDocumentChanges(active, draft) {
  ...
  const typeFieldLabels = {
    ...
    fields: 'Campos',
    titleSource: 'Título',
    validation: 'Reglas al validar',
    generation: 'Reglas al generar',
  }
  ...
  const generalRules = [
    { key: 'global', label: 'Voz al validar', path: 'global' },
    { key: 'prohibited', label: 'Restricciones adicionales', path: 'prohibited' },
    { key: 'generation.global', label: 'Voz al generar', path: 'generation.global' },
  ]
  const generalItems = generalRules
    .filter(({ key }) => {
      const beforeValue = key === 'generation.global' ? before.generation?.global : before[key]
      const afterValue = key === 'generation.global' ? after.generation?.global : after[key]
