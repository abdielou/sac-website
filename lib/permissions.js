 ...
 *
 * Features: members, payments, articles, guides, media, ai, inquiries
 * Special: scan_inbox (dashboard action, not tied to a feature)
 */

// All features in the system
export const FEATURES = ['members', 'payments', 'articles', 'guides', 'media', 'ai', 'inquiries']
  ...
  if (!envValue.includes(':')) {
    envValue
      ...
      .forEach((email) => {
        users.set(
          email,
          FEATURES.map((f) => `read_${f}`)
        )
      })
    return users
  }

  // Always use semicolon as entry separator (commas are used within permissions)
 ...
 * Dashboard tab is visible only if user has read_members (it shows member stats).
 */
export function getAccessibleFeatures(email) {
  ...
  const features = []
  if (perms.has('read_members')) features.push('dashboard')
  for (const f of FEATURES) {
...
export function canPerformAction(email, action) {
  ...
  const LEGACY_MAP = {
    ...
    edit_media: 'write_media',
    delete_media: 'write_media',
  }
...
export function isAdmin(email) {
  return (
    hasPermission(email, 'write_members') &&
...
export function getUserRole(email) {
  ...
  if (
    perms.has('write_*') ||
    (perms.has('write_members') &&
      perms.has('write_payments') &&
      perms.has('write_articles') &&
      perms.has('write_guides'))
  ) {
    return 'admin'
  }
  return null
...
export class PermissionChecker {
  ...
  canPerformAction(email, action) {
    return canPerformAction(email, action)
  }
