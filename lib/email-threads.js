...
export function buildEmailThreads({
  ...
  // Gmail's spam verdict on each member's copy. A message that every member
  // mailbox flagged never reached an inbox, and Gmail purges Spam after 30
  // days, so nobody can act on it. Mixed verdicts keep the message.
  const verdicts = new Map()
  for (const c of memberCopies) {
    if (!c.messageId) continue
    const v = verdicts.get(c.messageId) ?? { spam: 0, ham: 0 }
    v[c.spam ? 'spam' : 'ham'] += 1
    verdicts.set(c.messageId, v)
  }
  const unanimousSpam = (id) => {
    const v = verdicts.get(id)
    return Boolean(v && v.spam > 0 && v.ham === 0)
  }
  let spamCount = 0
  ...
  for (const r of sortedInbound) {
    ...
    if (dropHeld && unanimousSpam(r.messageId)) {
      spamCount += 1
      continue
    }
    const thread = getThread(r.subject)
  ...
  for (const thread of threads.values()) {
    ...
    // With requireCopy, a thread is listed only when the reading mailbox
    // still holds one of its messages under the label. A deleted or purged
    // message has nothing to open, so it drops out.
    const sender = senderOf(thread, senders)
    if (requireCopy && !sender) {
      noCopyCount += 1
      continue
    }
    const seen = new Map()
    ...
    result.push({
      ...
      sender,
      messageId: [...thread.inboundIds].pop() ?? null,
      firstInboundAt,
      lastInboundAt,
