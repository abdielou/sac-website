...
import sharplessData from '../data/catalog/sharpless.json'
...
function loadCatalog() {
  if (!_catalog) {
    _catalog = [...curatedData, ...supplementaryData, ...sharplessData.standalone]
    // Apply Sharpless cross-link overlays onto existing NGC/IC records.
    // Mutates the shared imported record in place — safe: lib/catalog is the
    // only runtime consumer of the JSON, this runs once (guarded above), and
    // the spread only adds a `sharpless` key.
    const byId = new Map(_catalog.map((o) => [o.id, o]))
    for (const link of sharplessData.crossLinks) {
      const obj = byId.get(link.id)
      if (obj) {
        obj.catalogIds = { ...obj.catalogIds, sharpless: link.sharpless }
      }
    }
  }
  return _catalog
...
export function searchCatalog(query, options = {}) {
  ...
  for (const obj of catalog) {
    ...
    if (
      ...
      (ids.sharpless && normalizeCatalogId(ids.sharpless) === qNorm)
    ) {
      score = 100
    }
    // Exact name match
    ...
    // Partial catalog ID match (starts with)
      ...
      (ids.sharpless && normalizeCatalogId(ids.sharpless).startsWith(qNorm))
    ) {
      score = 70
    }
