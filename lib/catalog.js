...
function loadCatalog() {
  if (!_catalog) {
    const baseDir = join(dirname(fileURLToPath(import.meta.url)), '..', 'data', 'catalog')
    const curated = JSON.parse(readFileSync(join(baseDir, 'openngc-curated.json'), 'utf-8'))
    const supplementary = JSON.parse(readFileSync(join(baseDir, 'supplementary.json'), 'utf-8'))
    _catalog = [...curated, ...supplementary]
  }
  return _catalog
}
