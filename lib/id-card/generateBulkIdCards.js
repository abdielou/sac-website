...
import { generateQrDataUrl, cropToSquare } from './generateIdCard'
...
export async function generateBulkIdCardsPdf(members) {
  ...
  const logoFilePath = path.join(process.cwd(), 'public/static/images/sac-main-logo.png')
  const logoPath = `data:image/png;base64,${fs.readFileSync(logoFilePath).toString('base64')}`
  ...
  const bgFilePath = path.join(process.cwd(), `public/static/images/id-bg-${currentYear}.png`)
  const backgroundPath = fs.existsSync(bgFilePath)
    ? `data:image/png;base64,${fs.readFileSync(bgFilePath).toString('base64')}`
    : null
