...
export const LAYOUT = {
  ...
  infoColumn: {
    ...
    pillsRow: { gap: s(3), marginTop: s(3) },
    designationPill: {
      height: s(11),
      borderRadius: s(5.5),
...
/**
 * Build card data for a family member ID card.
 * Vigencia inherits from the primary (sponsoring) member.
 */
export function buildFamilyCardData({ primaryMember, familyDisplayName, images }) {
  let vigenciaYear
  if (primaryMember.expirationDate) {
    vigenciaYear = new Date(primaryMember.expirationDate).getUTCFullYear()
  } else {
    vigenciaYear = new Date().getUTCFullYear()
  }

  const trimmed = (familyDisplayName || '').trim()
  const parts = trimmed.split(/\s+/).filter(Boolean)
  const firstLine = parts[0] || ''
  const lastLine = parts.slice(1).join(' ')

  return { firstLine, lastLine, vigenciaYear, designation: 'Familiar', images }
}
