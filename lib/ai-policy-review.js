...
import { getImageGenerationConfig, parseOpenRouterImageResponse } from './ai-image-generation'
import {
...
async function evaluateWithOpenRouter(
  ...
  for (let attempt = 0; attempt < POLICY_REVIEW_MAX_ATTEMPTS; attempt += 1) {
    ...
    if (!response?.ok) {
      ...
      lastErrorCode = retryable ? 'provider_error' : 'provider_rejection'
    ...
    if (typeof assistantText !== 'string' || !assistantText.trim()) {
      lastErrorCode = parseOpenRouterImageResponse(data) ? 'wrong_modality' : 'empty_response'
      continue
    }
