...
import { generateOpenRouterText } from './ai-openrouter-sdk'
...
async function evaluateWithOpenRouter(
  ...
  const configuredModel = dependencies.model || getConfiguredOpenRouterModels().textModel
  const model = resolveOpenRouterModels(configuredModel).textModel
  ...
  for (let attempt = 0; attempt < POLICY_REVIEW_MAX_ATTEMPTS; attempt += 1) {
    let response
    try {
      response = await generateOpenRouterText({
        apiKey,
        fetchImpl,
        model,
        messages,
        temperature: 0,
        forceJson: true,
        maxOutputTokens: POLICY_REVIEW_MAX_TOKENS,
        timeoutMs: POLICY_REVIEW_TIMEOUT_MS,
      })
    } catch (error) {
      lastErrorCode = error?.openRouterErrorCode || 'network_error'
      if (shouldRetryOpenRouterOperation(error)) continue
      break
    }
    ...
    if (typeof assistantText !== 'string' || !assistantText.trim()) {
      lastErrorCode = response.hasImage ? 'wrong_modality' : 'empty_response'
      continue
    }
 ...
 * violations are rejected locally; all other requests use the resolved
 * OpenRouter text model. Transient failures retry once; every remaining
...
/**
 * Reviews generated or validated text plus every supplied image with the
 * vision-capable OpenRouter text model resolved for internal operations.
 */
