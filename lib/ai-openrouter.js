 ...
 *   forceJson?: boolean,
 *   modalities?: string[],
 *   imageConfig?: { aspect_ratio?: string, [key: string]: unknown },
 * }} options
 */
...
/**
 * Build a text-output chat/completions body. Output modalities are owned by
 * this helper so callers cannot accidentally request an image or attach image
 * generation settings to a text operation.
 * @param {{
 *   model: string,
 *   messages: object[],
 *   temperature?: number,
 ...
 * }} options
 */
export function buildOpenRouterTextChatBody({ model, messages, temperature, forceJson = false }) {
  return buildOpenRouterChatBody({
    model,
    messages,
    temperature,
    forceJson,
...
/**
 * Build an image-generation chat/completions body. OpenRouter image requests
 * also retain text output so the provider can return accompanying metadata.
 * @param {{
 *   model: string,
 *   messages: object[],
 *   temperature?: number,
 ...
 * }} options
 */
export function buildOpenRouterImageChatBody({
  model,
  messages,
  temperature,
  forceJson = false,
