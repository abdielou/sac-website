...
async function fetchAccountability() {
  ...
  // One query covers both the group's own inbound records and the copies
  // delivered to each member. Member copies carry Gmail's spam verdict.
  const received = await fetchGmailLogRecords({
    ...window,
    userKey: 'all',
