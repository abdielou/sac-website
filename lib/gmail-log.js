...
import { getCachedData, CACHE_KEYS } from './cache.js'
import { buildEmailThreads } from './email-threads.js'
import { fetchSenders, createGmailAuth } from './gmail-senders.js'
...
// The Reports API rejects bursts with "Request rate higher than configured".
// Every window query from every event type shares this ceiling.
...
function normalizeActivity(activity) {
  ...
  for (const event of activity.events ?? []) {
    ...
    records.push({
      ...
      spam: params['message_info.is_spam'] === true,
    })
  }
  return records
}
...
// A shared gate across all concurrent window queries. Slots are handed
// straight to the next waiter so the ceiling is never exceeded.
let activeQueries = 0
const waiting = []
...
/**
 * True for the quota errors worth retrying. Google reports them as 429, or
 * as 403 with a rate or quota reason.
 * @param {any} error
 */
export function isRateLimitError(error) {
  ...
  return /rate|quota|limit/i.test(`${reason} ${error?.message ?? ''}`)
}

const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms))

/**
 * One request through the shared gate, retried with backoff on quota errors.
 */
async function requestThrottled(auth, url) {
  await acquireSlot()
  try {
    for (let attempt = 0; ; attempt += 1) {
      try {
        return await auth.request({ url })
      } catch (error) {
        if (attempt >= RETRY_DELAYS_MS.length || !isRateLimitError(error)) throw error
...
async function fetchWindow({ auth, userKey, mailEventType, startTime, endTime }) {
  ...
  do {
    ...
    const res = await requestThrottled(auth, url.toString())
...
async function fetchAccountability() {
  ...
  // Every source runs concurrently. Run in sequence this exceeds the
  // serverless time limit; the slowest source alone sets the wall clock.
  // The received query covers both the group's own inbound records and the
  // copies delivered to each member, which carry Gmail's spam verdict.
  const [received, replied, sent, { senders, senderStatus }] = await Promise.all([
    fetchGmailLogRecords({ ...window, userKey: 'all', mailEventType: MAIL_EVENT.RECEIVED }),
    fetchGmailLogRecords({ ...window, userKey: 'all', mailEventType: MAIL_EVENT.REPLIED }),
    fetchGmailLogRecords({ ...window, userKey: 'all', mailEventType: MAIL_EVENT.SENT }),
    loadSenders(start.toISOString()),
  ])
  const inbound = received.filter((r) => r.actor === groupAddress)
  const memberCopies = received.filter((r) => r.actor !== groupAddress)
  ...
  return buildEmailThreads({
    ...
    senders,
    senderStatus,
    // Only list what the mailbox still holds, when the mailbox was readable.
    requireCopy: senderStatus.ok,
    groupAddress,
    now: end,
...
export async function getEmailAccountability(forceRefresh = false) {
  return getCachedData(CACHE_KEYS.INQUIRIES, fetchAccountability, forceRefresh)
}
