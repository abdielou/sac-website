...
import { sanitizeMemberProfileFields } from './member-profile-fields'
...
async function getSpreadsheet() {
  if (!docInstance) {
    ...
    // Refresh sheet metadata so rowCount reflects any rows added since the doc was first loaded.
    // Without this, getRows() uses a stale rowCount and silently drops recently-appended rows.
  ...
  return getCachedData(
    ...
    async () => {
      return withRetry(async () => {
        ...
        // Pass an explicit limit large enough to cover any realistic sheet size,
        // so we never rely on the cached rowCount which can lag behind rows
        // appended externally (e.g. via Apps Script).
        const [rows, { emailIndex, phoneIndex }] = await Promise.all([
          sheet.getRows({ limit: 5000 }),
        ...
        return rows.map((row, index) => {
          ...
          // Family members (semicolon-separated names + JSON photo map)
          const familyMembersRaw = row.get('familyMembers') || ''
          const familyMemberPhotosRaw = row.get('familyMemberPhotos') || ''
          ...
          return {
            ...
            dataStatus,
            photoFileId,
            familyMembers: parseFamilyMembers(familyMembersRaw),
            familyMemberPhotos: parseFamilyMemberPhotos(familyMemberPhotosRaw),
...
export async function writeGeoData(updates) {
  return withRetry(async () => {
    ...
    if (!hasGeoLat || !hasGeoLng) {
      ...
      await ensureSheetColumnCapacity(sheet, newHeaders.length)
...
/**
 * Expand worksheet grid if needed before adding header columns.
 * @param {import('google-spreadsheet').GoogleSpreadsheetWorksheet} sheet
 * @param {number} requiredColumnCount
 */
async function ensureSheetColumnCapacity(sheet, requiredColumnCount) {
  if (sheet.columnCount >= requiredColumnCount) return
  await sheet.resize({
...
/**
 * Ensure named columns exist in row 1, resizing the sheet if needed.
 * @param {import('google-spreadsheet').GoogleSpreadsheetWorksheet} sheet
 * @param {string[]} columnNames
 */
async function ensureHeaderColumns(sheet, columnNames) {
  await sheet.loadHeaderRow()
  const headers = sheet.headerValues
  const newHeaders = [...headers]
  let changed = false
...
/**
 * Ensure familyMembers and familyMemberPhotos columns exist on the CLEAN sheet.
 * @param {import('google-spreadsheet').GoogleSpreadsheetWorksheet} sheet
 */
async function ensureFamilyColumns(sheet) {
  await ensureHeaderColumns(sheet, ['familyMembers', 'familyMemberPhotos'])
}

/**
 * Find a member row by email or sacEmail.
 * @param {Array} rows
 * @param {string} email
 */
function findMemberRowByEmail(rows, email) {
  const normalizedEmail = email.trim().toLowerCase()
  return rows.find((row) => {
    const rowEmail = (row.get('E-mail') || row.get('email') || row.get('Email') || '')
      .trim()
      .toLowerCase()
    const rowSacEmail = (row.get('sac_email') || row.get('SAC_Email') || '').trim().toLowerCase()
    return rowEmail === normalizedEmail || rowSacEmail === normalizedEmail
  })
}

/**
 * Update the family member names list for a member.
 * Prunes photo map keys that are no longer in the names list.
 *
 * @param {string} email - Member email (used to find the row)
 * @param {string[]} names - Ordered family member display names
 * @returns {Promise<{success: boolean}>}
 */
export async function updateFamilyMembers(email, names) {
  return withRetry(async () => {
    const doc = await getSpreadsheet()
    const sheet = doc.sheetsByTitle['CLEAN']
    if (!sheet) throw new Error('CLEAN sheet not found in spreadsheet')

    await ensureFamilyColumns(sheet)

    const rows = await sheet.getRows()
    const targetRow = findMemberRowByEmail(rows, email)
    ...
    if (sanitizeError) throw new Error(sanitizeError)

    const previousNames = parseFamilyMembers(targetRow.get('familyMembers') || '')
    targetRow.set('familyMembers', serializeFamilyMembers(sanitizedNames))

    const currentPhotos = parseFamilyMemberPhotos(targetRow.get('familyMemberPhotos') || '')
    const migratedPhotos = migrateFamilyMemberPhotos(previousNames, sanitizedNames, currentPhotos)
    targetRow.set('familyMemberPhotos', serializeFamilyMemberPhotos(migratedPhotos))
...
/**
 * Store a Google Drive photo file ID for a family member by display name.
 *
 * @param {string} email - Member email (used to find the row)
 * @param {string} familyName - Exact display name from familyMembers list
 * @param {string} photoFileId - Google Drive file ID
 * @returns {Promise<{success: boolean}>}
 */
export async function updateFamilyMemberPhoto(email, familyName, photoFileId) {
  return withRetry(async () => {
    const doc = await getSpreadsheet()
    const sheet = doc.sheetsByTitle['CLEAN']
    if (!sheet) throw new Error('CLEAN sheet not found in spreadsheet')
    ...
    const photos = parseFamilyMemberPhotos(targetRow.get('familyMemberPhotos') || '')
    photos[familyName.trim()] = photoFileId
    targetRow.set('familyMemberPhotos', serializeFamilyMemberPhotos(photos))
...
export async function updateMemberProfile(email, fields) {
  return withRetry(async () => {
    ...
    if (!headers.includes('photoFileId')) {
      const newHeaders = [...headers, 'photoFileId']
      await ensureSheetColumnCapacity(sheet, newHeaders.length)
      await sheet.setHeaderRow(newHeaders)
      await sheet.loadHeaderRow()
    }
    ...
    const sanitizedFields = sanitizeMemberProfileFields(fields)
    ...
    const locationChanged = locationFields.some((f) => sanitizedFields[f] !== undefined)

    for (const [field, column] of Object.entries(fieldMap)) {
      if (sanitizedFields[field] !== undefined) {
        targetRow.set(column, sanitizedFields[field])
      }
    }
...
export { getSpreadsheet, MEMBERSHIP_FEE, calculateMembershipStatus, normalizePhone }
