...
async function getSpreadsheet() {
  if (!docInstance) {
    ...
    docInstance = new GoogleSpreadsheet(process.env.GOOGLE_SHEET_ID, auth)
  ...
  return getCachedData(
    ...
    async () => {
      return withRetry(async () => {
        ...
        return rows.map((row, index) => {
          // Payment fields - exact columns depend on spreadsheet structure
          const email = row.get('Email') || row.get('email') || ''
          const amount = row.get('Amount') || row.get('Monto') || ''
          const date = row.get('Date') || row.get('Fecha') || ''
          const source = row.get('Source') || row.get('Fuente') || ''
          ...
          return {
            ...
            date,
            source: source.toLowerCase().replace(/\s+/g, '_'), // 'ATH Movil' -> 'ath_movil'
            _raw: row.toObject(),
          }
        })
      })
