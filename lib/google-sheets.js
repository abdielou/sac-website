  ...
  return getCachedData(
    ...
    async () => {
      return withRetry(async () => {
        ...
        // DEBUG: log row count and check for specific member
        console.log(`[getMembers] rows returned: ${rows.length}`)
        const debugRow = rows.find((r) => {
          const e = r.get('E-mail') || r.get('email') || r.get('Email') || ''
          return e.toLowerCase().includes('tiffany')
        })
        if (debugRow) {
          console.log(`[getMembers] found tiffany row: email=${debugRow.get('E-mail')} nombre=${debugRow.get('Nombre')} apellidos=${debugRow.get('Apellidos')}`)
        } else {
          console.log('[getMembers] tiffany NOT found in rows')
        }
