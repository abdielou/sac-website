...
import { DEFAULT_VERSION, getDefaultGuidelines } from './ai-guidelines'
...
import { normalizeGuidelineVersionName } from './guideline-version-name'
 ...
 * @property {Array<{ version: string, versionName?: string|null, activatedAt: string, activatedBy: string }>} versions
...
export function nextPublishedVersion(existingVersions = []) {
  ...
  for (const entry of existingVersions) {
    ...
    const match = String(version || '').match(/^(?:default-)?v(\d+)$/)
    if (match) {
...
async function readDraftRecord(metaDraft) {
  ...
  const record = await getJson(KEYS.draftRevision(metaDraft.id, revision))
  return record
...
async function publishActivationDocument(
  meta,
  document,
  { draftId, revision, publishedBy, versionName }
) {
...
/**
 * Ensure bucket has at least the default guidelines as the active published version.
 * No-op when S3 is not configured.
 */
async function ensureGuidelinesSeeded({ seededBy = SYSTEM_ACTOR } = {}) {
  ...
  const version = seed.version || DEFAULT_VERSION
    ...
    const nextMeta = queueAuditEvents(
      {
        ...
        versions: [
          {
            version,
            versionName: seed.versionName || null,
            activatedAt,
    ...
    return [
      {
        ...
        versionName: defaults.versionName || null,
        activatedAt: null,
        activatedBy: null,
        status: 'active',
      },
...
export async function getGuidelineVersion(version) {
  ...
  return doc ? normalizeGuidelineDocumentV3(doc) : null
}

async function getGuidelineDraft() {
  if (!getBucket()) return null
...
async function getGuidelineAuditLog() {
  if (!getBucket()) return []
...
export async function createGuidelineDraft({ createdBy, basedOnVersion } = {}) {
  ...
  await ensureGuidelinesSeeded({ seededBy: createdBy || SYSTEM_ACTOR })
  ...
  const document = normalizeGuidelineDocumentV3(cloneGuidelines(sourceDoc))
  const revision = 1
...
export async function activateGuidelineVersion(
  draftId,
  { activatedBy, expectedRevision, versionName } = {}
) {
  if (!getBucket()) {
    throw new Error('S3_ARTICLES_BUCKET_NAME no está configurado')
  }
  ...
  await flushPendingAuditEvents()
  const normalizedVersionName = normalizeGuidelineVersionName(versionName) || null
  let snapshot = await readMetaSnapshot()
  const { meta } = snapshot
  ...
  if (completed) {
    ...
    return {
      active: normalizeGuidelineDocumentV3(existing),
  ...
  const activationErrors = validation.errors
  ...
  const publishedVersionName = published.versionName || normalizedVersionName
  ...
  const auditEvents = [
    createAuditEvent({
      ...
      version,
      by,
      detail: publishedVersionName
        ? `Versión ${version} activada: ${publishedVersionName}`
        : `Versión ${version} activada`,
    }),
...
export async function rollbackGuidelineVersion(version, { rolledBackBy } = {}) {
  ...
  for (let attempt = 0; attempt < MAX_STATE_WRITE_ATTEMPTS; attempt += 1) {
    ...
    const known = (snapshot.meta.versions || []).find((entry) => entry.version === version)
    if (!known) {
      const err = new Error('Versión no encontrada')
      err.code = 'VERSION_NOT_FOUND'
      throw err
  ...
  return { active: normalizeGuidelineDocumentV3(doc), auditLog }
}
