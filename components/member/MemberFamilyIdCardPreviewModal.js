...
/**
 * Modal to preview a family member ID card before downloading (member profile).
 *
 * @param {{ isOpen: boolean, onClose: () => void, familyDisplayName: string | null }} props
 */
export function MemberFamilyIdCardPreviewModal({ isOpen, onClose, familyDisplayName }) {
  const [profile, setProfile] = useState(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState(null)

  useEffect(() => {
    if (!isOpen || !familyDisplayName) {
      setProfile(null)
      setError(null)
      return
    }
    ...
    fetch(`/api/member/family/${encodeURIComponent(familyDisplayName)}/id-card-preview`)
      .then(async (res) => {
        if (!res.ok) {
  ...
  }, [isOpen, familyDisplayName])

  if (!isOpen || !familyDisplayName) return null
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-sm w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-6 py-6 flex flex-col items-center gap-4">
          ...
          <div className="flex gap-2 w-full justify-center pt-2">
            ...
            {canDownload && (
              <a
                href={`/api/member/family/${encodeURIComponent(familyDisplayName)}/id-card`}
                target="_blank"
...
export default MemberFamilyIdCardPreviewModal
