...
export const POST = auth(async function POST(req, { params }) {
  ...
  // PRD path: /guidelines/:version/activate — for draft activation the
  // :version segment carries the draft id (draft_…).
  const draftId = resolvedParams?.version
  if (!draftId) {
  ...
  try {
    ...
    console.error('POST /api/admin/ai/guidelines/[version]/activate failed', error)
    return NextResponse.json(
